...
class TelethonRedisIntegration:
    ...
    def __init__(
        ...
        logger.info("Telethon-Redis integration initialized")
    ...
    async def _handle_telegram_message(self, event):
        ...
        try:
            ...
            if success:
                ...
                logger.debug(f"📤 Published message from {enriched_data.get('channel_title')} (priority: {priority.name})")
            else:
    ...
    async def _enrich_message_data(self, message_data: Dict[str, Any], channel_metadata: Dict) -> Dict[str, Any]:
        ...
        # Add geopolitical relevance flags (can be enhanced with AI)
        enriched["relevance_flags"] = await self._analyze_geopolitical_relevance(text)
        
        return enriched
    
    async def _analyze_geopolitical_relevance(self, text: str) -> Dict[str, Any]:
        """Analyze text for geopolitical relevance (basic keyword matching)."""
        geopolitical_keywords = {
            "conflicts": ["war", "conflict", "battle", "invasion", "attack", "military"],
            "diplomacy": ["treaty", "agreement", "summit", "negotiation", "ambassador"],
            "economics": ["sanctions", "trade", "tariff", "economy", "gdp", "inflation"],
            "regions": ["ukraine", "russia", "china", "middle east", "nato", "eu"],
            "security": ["terrorism", "cyber", "intelligence", "security", "threat"]
        }
        ...
        total_score = sum(cat["score"] for cat in relevance.values())
        relevance["overall_score"] = total_score
        relevance["is_relevant"] = total_score > 0
    ...
    async def _store_channel_metadata(self, channel_info: Dict, priority: Priority):
        ...
        try:
            # This would use the TelegramChannel model
            # Implementation depends on database schema
            logger.debug(f"Storing channel metadata for {channel_info.get('title')}")
        except Exception as e:
            logger.error(f"Failed to store channel metadata: {e}")
    
    async def _store_message_data(self, message_data: Dict[str, Any]):
        """Store message data in database asynchronously."""
        try:
            # This would use the TelegramMessage model
            # Implementation depends on database schema
            logger.debug(f"Storing message data for message {message_data.get('message_id')}")
        except Exception as e:
            logger.error(f"Failed to store message data: {e}")
...
async def initialize_integration(
    ...
    logger.info("Global Telethon-Redis integration initialized")
    return telethon_redis_integration
